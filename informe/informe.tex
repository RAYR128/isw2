...
\begin{longtable}{@{}p{0.9cm}p{3.9cm}p{2.3cm}p{2.8cm}p{3.8cm}@{}}
  ...
  \small{RF5} & Contratación de personal ejecutivo & Gerente / Director & \scriptsize\texttt{/contratos/ejecutivo} & \scriptsize\texttt{POST /api/contratos/ejecutivo} \\
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    % contratos_e
    \node[entity, below=2.8cm of contratos_p] (contratos_e) 
...
\begin{center}
  ...
  \texttt{/dashboard} $\rightarrow$ \texttt{/contratos/personal} \quad | \quad \texttt{/contratos/ejecutivo}
\end{center}
...
\begin{longtable}{@{}p{1.5cm}p{4cm}p{6.5cm}@{}}
  ...
  \midrule
  \endfirsthead
  GET & \scriptsize\texttt{/api/contratos/ejecutivo} & Lista todos los registros ejecutivos. \\
  POST & \scriptsize\texttt{/api/contratos/ejecutivo} & Inicia proceso de contratación. \\
  \bottomrule
\end{longtable}

\textbf{Ejemplo -- crear registro ejecutivo:}
\begin{lstlisting}[language=bash]
curl -X POST http://localhost:3000/api/contratos/ejecutivo \
  -H "Content-Type: application/json" \
  -d '{
