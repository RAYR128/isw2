...
\begin{itemize}
  \item API REST con 18 endpoints documentados y probados.
...
\begin{longtable}{@{}p{1.5cm}p{4.5cm}p{6.5cm}@{}}
  ...
  DELETE & \scriptsize\texttt{/api/contratos/personal/:id} & Elimina un contrato. \\
  \bottomrule
\end{longtable}
...
\textbf{Ejemplo -- eliminar contrato de personal:}
\begin{lstlisting}[language=bash]
curl -X DELETE http://localhost:3000/api/contratos/personal/1
\end{lstlisting}

\textbf{Respuesta esperada (eliminación exitosa):}
\begin{lstlisting}
{ "mensaje": "Contrato eliminado exitosamente" }
\end{lstlisting}

\textbf{Respuesta esperada (trabajador asignado, HTTP 409):}
\begin{lstlisting}
...
\begin{longtable}{@{}p{1.5cm}p{4cm}p{6.5cm}@{}}
  ...
  GET & \scriptsize\texttt{/api/contratos/ejecutivos/:id} & Detalle completo de un contrato ejecutivo. \\
  POST & \scriptsize\texttt{/api/contratos/ejecutivos/:id} & Modifica contrato existente (campos parciales). \\
  DELETE & \scriptsize\texttt{/api/contratos/ejecutivos/:id} & Elimina un contrato ejecutivo. \\
  \bottomrule
\end{longtable}
...
\textbf{Ejemplo -- consultar contrato ejecutivo:}
\begin{lstlisting}[language=bash]
curl http://localhost:3000/api/contratos/ejecutivos/1
\end{lstlisting}

\textbf{Ejemplo -- modificar contrato ejecutivo:}
\begin{lstlisting}[language=bash]
curl -X POST http://localhost:3000/api/contratos/ejecutivos/1 \
  -H "Content-Type: application/json" \
  -d '{
    "salario": 1100000,
    "prioridad": "Media",
    "comentarios": "Segunda entrevista programada"
  }'
\end{lstlisting}

\textbf{Ejemplo -- eliminar contrato ejecutivo:}
\begin{lstlisting}[language=bash]
curl -X DELETE http://localhost:3000/api/contratos/ejecutivos/1
\end{lstlisting}

\textbf{Respuesta esperada (eliminación exitosa):}
\begin{lstlisting}
{ "mensaje": "Contrato ejecutivo eliminado exitosamente" }
\end{lstlisting}

\pagebreak
\section{Resumen de endpoints}

En total el sistema expone \textbf{18 endpoints} bajo el prefijo \texttt{/api}, distribuidos en cuatro módulos: autenticación (1), asignaciones (7), contratos de personal (5) y contratos ejecutivos (5).
